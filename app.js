...
function overlayTasks(parsed, sourceName){
  ...
  parsed.forEach(p=>{                            // GRANULAR: one task per report item
    ...
    if(!ex){
      ...
      if(ex.wstatus && !inc.wstatus) inc.wstatus=ex.wstatus;    // a user-set status survives re-imports
...
function isClosed(t){ if(t.wstatus) return t.wstatus==='Done';   // a manually-set status is authoritative
  return (+t.progress||0)>=100; }   // coerce: progress may arrive as a string ("100")
function statusLine(t){
  const p=t.progress;
  if(t.wstatus) return `${t.wstatus} (${p||0}%)`;
...
function editTaskField(id, field, val){
  ...
  if(field==='progress'||field==='risk'||field==='complexity'||field==='nextProgress'||field==='due'||field==='wstatus') t.manualEdit=true;  // survive re-import
...
function memberBlock(name, list, isUnassigned, mid){
  ...
  const cards=list.length? `<div class="task-grid">${list.map((t,i)=>taskCard(t,i+1)).join('')}</div>` : '';   // #N = export order
...
function taskCard(t, seq){
  ...
  const ws=workStatus(t), wsCls=ws.toLowerCase().replace(/[^a-z]+/g,'-');
  ...
  return `<div class="card ${closed?'closed':''}" draggable="true" data-task="${t.id}" title="Drag to reorder — Word/PPTX follow this order">
    ...
    <div class="ct">${seq?`<span class="seq-badge" title="Export order — drag cards to change">#${seq}</span>`:''}<span class="proj">${esc(shortP)}</span></div>
    <div class="tcard-topic">${esc(taskTopic(t))}</div>
    <div class="tags">
      ${closed?'<span class="tag closed">✓ Done</span>':`<span class="tag ws-${wsCls}">${esc(ws)}</span>`}
  ...
  $('#taskModalInner').innerHTML=`
    ...
    <div class="modal-body detail">
      ...
      <div class="edit-row">
        ...
        <label>Status<select data-edit="wstatus" data-tid="${t.id}">
          <option value="" ${!t.wstatus?'selected':''}>Auto</option>
          ${WSTATUS.map(s=>`<option ${t.wstatus===s?'selected':''}>${s}</option>`).join('')}
        </select></label>
      </div>
      <div class="kv">
...
function loadTaskImages(t){
  ...
  (t.images||[]).forEach((im,ix)=>{
    ...
    const sq=document.createElement('span'); sq.className='img-seq'; sq.textContent='#'+(ix+1); span.appendChild(sq);   // matches "Fig N" in the deck
...
function assemblePptx(memberIds){
  ...
  function groupStatus(g){
    if(g.w) return {text:g.w, color:({'Done':PPT.green,'Pending':PPT.amber,'At risk':'E5484D','On hold':PPT.gray,'In-progress':PPT.teal})[g.w]||PPT.teal};   // manual status wins
