...
function renderStats(){
  ...
  const cards=[
    ...
    ['📈', doneRate+'%', '完成率 Done', 'accent', 'members', 'closed'],
...
function deleteProjectGroup(projk){
  const g=projectGroups().find(x=>x.projk===resolveProjk(projk)); if(!g) return;
  if(!confirm('刪除專案「'+g.label+'」及其 '+g.tasks.length+' 筆任務？此動作無法復原。')) return;
  if(!confirm('再次確認：真的要刪除「'+g.label+'」嗎？')) return;
...
function catalogCard(g){
  ...
  // edit mode: category is part of the form, saved together with Customer/說明 on 儲存 (no premature re-render)
  ...
  return `<div class="proj-card cat-b-${esc(curCat)}" draggable="true" data-projk="${esc(g.projk)}">
    <div class="pc-head">
      ...
      <div class="pc-actions">
        ...
        <button class="btn sm danger" data-delproj="${esc(g.projk)}" title="刪除此專案與其任務">🗑</button>
      </div>
    </div>
    ${metaRow}
