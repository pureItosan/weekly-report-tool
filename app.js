...
function renderStats(){
  ...
  const doneRate=total?Math.round(closed/total*100):0;     // 完成率 = 已結案 / 總任務（不依賴成員自填進度，不會出現 NaN）
  const high=vt.filter(t=>t.risk==='High'&&!isClosed(t)).length;
  const newCnt=vt.filter(t=>t.status==='New').length;
  // each stat card doubles as navigation: [icon, value, label, colorClass, view, statusFilter].
  // view=null -> display-only (no click, no highlight) — used for 完成率 so it doesn't clash with 已結案.
  const cards=[
    ...
    ['📈', doneRate+'%', '完成率 Done', 'accent', null, ''],
    ['⚠️', high, '高風險 High risk', 'warn', 'members', 'highrisk'],
  ];
  $('#statsRow').innerHTML=cards.map(([ic,n,l,c,view,flt])=>{
    if(!view) return `<div class="stat ${c}"><div class="st-ic">${ic}</div><div><div class="num">${n}</div><div class="lbl">${l}</div></div></div>`;
...
async function addProjectImages(projk, fileList){
  ...
  if(!files.length){ toast(all.length?'這個檔案不是圖片':'沒有選到檔案'); return; }
  toast('處理圖片中…請稍候');
  const m=projMeta[projk]||{}; m.images=m.images||[]; let n=0;
  for(const f of files){ try{ const s=await shrinkImageBudget(await fileToDataURL(f), 2200); m.images.push({id:uid(), data:s.data, w:s.w, h:s.h}); n++; }catch(e){ console.warn('project image failed', e); } }
  if(!n){ toast('圖片處理失敗，請換個檔案試試'); return; }
  projMeta[projk]=m; persist(); renderProjImages(); renderCatalog(); toast('已新增 '+n+' 張圖片');
}
